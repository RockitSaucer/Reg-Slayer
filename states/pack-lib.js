  ...
  function standardLayers(spec) {
    ...
    if (hasUsfs(spec)) {
      out.push({
        ...
        notes: 'USFS in ' + spec.name + '. Confirm forest orders and ' + (spec.confirmLabel || spec.agency) + '.'
      });
    }
    if (spec.hasBlm) {
      out.push({
    ...
    if (wmaSpec && wmaSpec.url) {
      ...
      out.push({
        ...
        typeLabel: hl.short,
        url: PADUS,
        where: "State_Nm = '" + code + "' AND Pub_Access = 'OA' AND (Mang_Name = 'State Fish And Wildlife' OR Mang_Name = 'State Department of Natural Resources' OR Mang_Name = 'State Dept of Natural Resources' OR Des_Tp = 'HUNT')",
        useBbox: true, paginate: true,
        outFields: 'Mang_Name,Unit_Nm,Loc_Nm,State_Nm,Des_Tp,Pub_Access,GIS_Acres,OBJECTID',
        color: wma.color, fillOpacity: wma.fillOpacity, weight: wma.weight,
        listMode: 'unit',
    ...
    if (hasUsace(spec)) {
      out.push({
        ...
        url: PADUS,
        where: "Mang_Name = 'USACE' AND State_Nm = '" + code + "' AND Pub_Access = 'OA' AND (Des_Tp = 'REC' OR Des_Tp = 'PUB')",
        useBbox: true,
        outFields: 'Mang_Name,Unit_Nm,Loc_Nm,State_Nm,Des_Tp,Pub_Access,GIS_Acres,OBJECTID',
